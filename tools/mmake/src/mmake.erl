...
do_make_files(Worker, Fs, Opts) ->
    {T11, T12, _} = erlang:now(),
    {L, E, W} = process(Fs, Worker, lists:member(noexec, Opts), load_opt(Opts), 0, [], []),
    {T21, T22, _} = erlang:now(),
    case E of
        [] ->
            skip;
        _ ->
            io:format("~nErrors:~n", []),
            print_errors_warnings(E)
    end,
    case W of
        [] ->
            skip;
        _ ->
            ...
            print_errors_warnings(W)
    end,
    io:format("~n~p Files in ~p s~n", [L, T22 - T12 + (T21 - T11) * 1000000]).
...
%% 处理
process([{[], _Opts} | Rest], Worker, NoExec, Load, L, E, W) ->
    process(Rest, Worker, NoExec, Load, L, E, W);
process([{L, Opts} | Rest], Worker, NoExec, Load, C, E, W) ->
    Len = length(L),
    ...
    case catch do_worker(L, Opts, NoExec, Load, Worker2) of
        error ->
            error;
        {C1, E1, W1} ->
            process(Rest, Worker, NoExec, Load, C1 + C, E1 ++ E, W1 ++ W)
    end;
process([], _Worker, _NoExec, _Load, L, E, W) ->
    {L, E, W}.
...
do_worker(L, Opts, NoExec, Load, Worker) ->
    ...
    {C, E, W} = do_wait_worker(length(Pids), L2, Opts, NoExec, Load, Ref, 0, [], []),
    {C, E, W}.

%% @doc 一个文件编译完成后，立即进行下一个文件编译,不用等待
do_wait_worker(0, [], _Opts, _NoExec, _Load, _Ref, C, E, W) ->
    {C, E, W};
do_wait_worker(N, L, Opts, NoExec, Load, Ref, C, E, W) ->
    receive
        {false, Ref} ->
            case L of
                [H | T] ->
                    ...
                    do_wait_worker(N, T, Opts, NoExec, Load, Ref, C, E, W);
                [] ->
                    %% 等待所有结果返回
                    do_wait_worker(N - 1, [], Opts, NoExec, Load, Ref, C, E, W)
            end;
        {{ok, _, W1}, Ref} ->
            case L of
                [H | T] ->
                    %% 未编译完成，编译后面的文件
                    start_worker(H, Opts, NoExec, Load, self(), Ref),
                    do_wait_worker(N, T, Opts, NoExec, Load, Ref, C + 1, E, W1 ++ W);
                [] ->
                    %% 等待所有结果返回
                    do_wait_worker(N - 1, [], Opts, NoExec, Load, Ref, C + 1, E, W1 ++ W)
            end;
        {{error, E1, W1}, Ref} ->
            case L of
                [H | T] ->
                    %% 未编译完成，编译后面的文件
                    start_worker(H, Opts, NoExec, Load, self(), Ref),
                    do_wait_worker(N, T, Opts, NoExec, Load, Ref, C + 1, E1 ++ E, W1 ++ W);
                [] ->
                    %% 等待所有结果返回
                    do_wait_worker(N - 1, [], Opts, NoExec, Load, Ref, C + 1, E1 ++ E, W1 ++ W)
            end;
        {error, Ref} ->
            throw(error);
        {'EXIT', _P, _Reason} ->
...
recompile(File, false, noload, Opts) ->
    io:format("~ts~n", [File]),
    compile:file(File, [return_errors, return_warnings, error_summary | Opts]);
...
print_errors_warnings([]) ->
    ok;
print_errors_warnings([{M, I} | R]) ->
    [io:format("    ~p:~p:~p~n", [M, L, E]) || {L, _, E} <- I],
    print_errors_warnings(R).