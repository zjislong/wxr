...
init([]) ->
    ...
    case cache:get(?battle_manager, {0, status_time(0)}) of
        {Status, Time} ->
...
terminate(_Reason, State) ->
    cache:put(?battle_manager, {State#state.status, State#state.time}),
    ok.
...
status_change(1) ->
    ...
    2;
status_change(2) ->
    gen_server:call(rank_manager_srv, {mfa, rank_lib,reset,["pvp"]}),
    0.
...
match() ->
    ...
    match(lists:keysort(2, sets:to_list(KeyList)), {-1, []}).
...
match([?BATTLE_MATCH_LIST(Key) | Keys], {LastKey, Rest}) ->
    dictionary:key_erase(?BATTLE_MATCH_LIST(LastKey)),
...
match1([{_, ?BATTLE_MATCH_PLAYER(ID)} | R]) ->
    ...
    supervisor:start_child(battle_sup, [ID, TarID]),
    match1(Rest).
...
-spec send_battle_info(IDs::[string()], State::#state{}) -> no_return().
...
-spec send_battle_info_1(State::#state{}, StarInfo::[#p_star_info{}], Now::pos_integer())-> no_return().
send_battle_info_1(State, StarInfo, Now)->
    {ok, Players} = lbm_kv:match_key(player, '_'),
    [send_battle_info_1(Player, State, StarInfo, Now)||{_, Player}<-Players].

-spec send_battle_info_1(#player{}|string(), State::#state{}, StarInfo::[#p_star_info{}], Now::pos_integer())-> no_return().
